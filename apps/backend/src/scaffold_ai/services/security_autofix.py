...
class SecurityAutoFix:
    ...
    def analyze_and_fix(self, graph: Dict) -> Tuple[Dict, List[str]]:
        ...
        for storage in storage_nodes:
            ...
            if not config.get("encryption"):
                ...
                label = storage.get("data", {}).get(
                    "label", storage.get("id", "unknown")
                )
    ...
    def get_security_score(self, graph: Dict) -> Dict:
        ...
        max_score += 25
        storage_nodes = [
            n for n in nodes if n.get("data", {}).get("type") in ["storage", "database"]
        ]
        if storage_nodes:
            encrypted = sum(
                1
                for n in storage_nodes
                if n.get("data", {}).get("config", {}).get("encryption")
            )
        ...
        if queue_nodes:
            with_dlq = sum(
                ...
                if n.get("data", {}).get("config", {}).get("has_dlq")
            )
        ...
        max_score += 15
        vpc_nodes = [
            n for n in nodes if n.get("data", {}).get("config", {}).get("vpc_enabled")
        ]
        if vpc_nodes:
        ...
        monitored = [
            n
            for n in nodes
            if n.get("data", {}).get("config", {}).get("monitoring_enabled")
        ]
        if monitored:
